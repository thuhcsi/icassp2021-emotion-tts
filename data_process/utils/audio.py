...
def resample(hp, src, dst):
    wav, sr = load_wav(hp, src)
    save_wav(hp, wav, dst)
...
def inv_spectrogram(hp, spectrogram):
    ...
    # S = _db_to_amp(spectrogram + hp.ref_level_db)  # Convert back to linear
...
def melspectrogram(hp, y, norm=True):
    y = parse_y(hp, y)
    if hp.preemphasis is None:
        D = _stft(hp, y)
    else:
    ...
    return _normalize(hp, S) if norm else S   # dtype float64


def mfcc(hp, y):
    log_mel = melspectrogram(hp, y, norm=False)
    mfcc = librosa.feature.mfcc(S=log_mel, n_mfcc=hp.num_mfcc // 3)
    ...
    return (mfcc - mfcc_min) * (mfcc_max - mfcc_min)
...
if __name__ == "__main__":
    hp = type('HP', (), dict(sample_rate=16000))
    pass
