...
def train(log_dir, args):
    ...
    log('Using model: emogst tacotron2')
    ...
    with tf.name_scope('model'):
        ...
        model(feats['inputs'],
              ...
              emo_labels=feats['soft_emo_labels'],
              training=training)
        model.add_loss()
    ...
    with tf.Session(config=config) as sess:
        try:
            ...
            """
            fetches = [global_step, model.optimize, model.loss, model.mel_loss,
                       model.spec_loss, model.stop_loss, model.emo_loss, model.mel_grad_norms_max,
                       model.spec_grad_norms_max, model.stop_grad_norms_max, model.emo_grad_norms_max]
            """
            fetches = [global_step, model.optimize, model.loss, model.mel_loss,
                       model.spec_loss, model.stop_loss, model.emo_loss]
            for _ in range(_max_step):
                start_time = time.time()
                # sess.run(debug.get_ops())
                # step, _, loss, mel_loss, spec_loss, stop_loss, emo_loss, mel_g, spec_g, stop_g, emo_g = sess.run(fetches)
                step, _, loss, mel_loss, spec_loss, stop_loss, emo_loss = sess.run(fetches)
                time_window.append(time.time() - start_time)
                loss_window.append(loss)
                """
                message = 'Step %-7d [%.3f sec/step,ml=%.3f,spl=%.3f,sl=%.3f,el=%.3f,mg=%.4f,spg=%.4f,sg=%.4f,eg=%.4f]' % (
                    step, time_window.average, mel_loss, spec_loss, stop_loss, emo_loss, mel_g, spec_g, stop_g, emo_g)
                """
                message = 'Step %-7d [%.3f sec/step,ml=%.3f,spl=%.3f,sl=%.3f,el=%.3f]' % (
                    step, time_window.average, mel_loss, spec_loss, stop_loss, emo_loss)
...
def main():
    ...
    log_dir = 'emogst_logs' + ('_' + args.log if args.log else '')
